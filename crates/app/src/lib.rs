...
async fn spawn_jobs(pool: &SqlitePool, config: &Config) {
    ...
    tokio::spawn(async move {
        ...
        let mut interval =
            tokio::time::interval(Duration::from_secs(config.scanner_poll_interval()));
        loop {
            interval.tick().await;
    ...
    tokio::spawn(async move {
        ...
        let mut interval =
            tokio::time::interval(Duration::from_secs(config.telegram_poll_interval()));
        loop {
            interval.tick().await;
    ...
    tokio::spawn(async move {
        ...
        let mut interval =
            tokio::time::interval(Duration::from_secs(config.distiller_poll_interval()));
        loop {
            interval.tick().await;
