...
async fn upsert_creates_and_updates() {
    ...
    let fetched = distilled_token::get_by_address(&pool, "addr1")
        .await
        .unwrap()
        .unwrap();
...
async fn get_by_address_returns_none_for_missing() {
    let pool = setup_memory_pool().await;
    let result = distilled_token::get_by_address(&pool, "missing")
        .await
        .unwrap();
...
async fn update_ranking_score_modifies() {
    ...
    assert!(
        distilled_token::update_ranking_score(&pool, "addr_upd", 99.0)
            .await
            .unwrap()
    );
    let fetched = distilled_token::get_by_address(&pool, "addr_upd")
        .await
        .unwrap()
        .unwrap();
...
async fn count_returns_total() {
    let pool = setup_memory_pool().await;
    distilled_token::upsert(&pool, &sample_token("a"))
        .await
        .unwrap();
    distilled_token::upsert(&pool, &sample_token("b"))
        .await
        .unwrap();
...
async fn list_narrative_clusters_distinct() {
    ...
    let clusters = distilled_token::list_narrative_clusters(&pool)
        .await
        .unwrap();
