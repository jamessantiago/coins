...
async fn get_or_create_channel_creates_new() {
    ...
    let (ch, created) = telegram::get_or_create_channel(&pool, "test_channel").await.unwrap();
    assert!(created);
...
async fn get_or_create_channel_returns_existing() {
    let pool = setup_memory_pool().await;
    telegram::get_or_create_channel(&pool, "existing").await.unwrap();
    let (ch, created) = telegram::get_or_create_channel(&pool, "existing").await.unwrap();
    assert!(!created);
...
async fn toggle_channel_enabled_flips() {
    ...
    telegram::toggle_channel_enabled(&pool, ch.id).await.unwrap();
    let toggled = telegram::get_channel_by_id(&pool, ch.id).await.unwrap().unwrap();
    assert!(!toggled.enabled);

    telegram::toggle_channel_enabled(&pool, ch.id).await.unwrap();
    let toggled_back = telegram::get_channel_by_id(&pool, ch.id).await.unwrap().unwrap();
...
async fn delete_channel_removes() {
    ...
    assert!(telegram::get_channel_by_id(&pool, ch.id).await.unwrap().is_none());
}
...
async fn message_lifecycle() {
    let pool = setup_memory_pool().await;
    let (ch, _) = telegram::get_or_create_channel(&pool, "msg_ch").await.unwrap();
    ...
    let with_addrs = telegram::iterate_messages_with_addresses(&pool).await.unwrap();
