...
async fn upsert_creates_and_updates() {
    ...
    poll_timestamp::upsert(&pool, "scanner", now, 5)
        .await
        .unwrap();

    let row = poll_timestamp::get_by_service(&pool, "scanner")
        .await
        .unwrap()
        .unwrap();
    ...
    poll_timestamp::upsert(&pool, "scanner", now, 10)
        .await
        .unwrap();
    let row = poll_timestamp::get_by_service(&pool, "scanner")
        .await
...
async fn get_by_service_returns_none_for_missing() {
    let pool = setup_memory_pool().await;
    let result = poll_timestamp::get_by_service(&pool, "nonexistent")
        .await
        .unwrap();
...
async fn list_all_returns_all_services() {
    let pool = setup_memory_pool().await;
    let now = NaiveDateTime::default();
    poll_timestamp::upsert(&pool, "scanner", now, 1)
        .await
        .unwrap();
    poll_timestamp::upsert(&pool, "distiller", now, 2)
        .await
        .unwrap();
