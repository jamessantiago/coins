...
CREATE INDEX IF NOT EXISTS idx_cluster_counts_lookup ON cluster_counts(cluster, bucket);
...
CREATE INDEX IF NOT EXISTS idx_known_pools_base_mint ON known_pools(base_mint);
...
CREATE INDEX IF NOT EXISTS idx_pump_bonding_curves_mint ON pump_bonding_curves(mint);
